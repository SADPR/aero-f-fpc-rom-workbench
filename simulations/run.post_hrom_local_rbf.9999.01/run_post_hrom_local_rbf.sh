...
if [[ ! -f ../run.hrom_local_rbf.9999.01/postpro/ReducedCoords.out ]]; then
  echo "ERROR: Missing ../run.hrom_local_rbf.9999.01/postpro/ReducedCoords.out"
  exit 1
fi

command -v module >/dev/null 2>&1 && module load cmake/3.8.1 gcc/9.1.0 openmpi/4.1.2 imkl/2019
/usr/bin/mpirun.openmpi -np "$NP" "$AEROF" FluidFile |& tee log_postpro_hrom_local_rbf.out
