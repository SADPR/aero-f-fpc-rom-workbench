...
NPOD=9999
TAU=01
DIR1=data.hrom_local_rbf."$NPOD"."$TAU"
DIR2=simulations/run.offline_local_rbf."$NPOD"."$TAU"/nonlinearrom

mkdir -p "$DIR1"

if [[ ! -f "$DIR2/gappy.top" ]]; then
  echo "ERROR: Missing $DIR2/gappy.top"
  echo "Run simulations/run.offline_local_rbf.$NPOD.$TAU/run_hyper_local_rbf.sh first."
  exit 1
fi
